    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          />
          <span className="text-xs font-medium px-2.5 py-1 rounded-full whitespace-nowrap bg-surface-sunken text-text-2 ring-1 ring-border/60">
        ...
        {isProp ? (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <Input type="number" step={10000} min={0} value={asset.amountCny ?? 0}
                onChange={e => updateAsset(asset.id, a => { a.amountCny = Number(e.target.value) || 0; })}
                className="h-8 text-right" />
            </div>
            <div>
              <Label className="block mb-1">月物业费 ¥</Label>
              <Input type="number" step={100} min={0} value={asset.monthlyMaintenance ?? 500}
                onChange={e => updateAsset(asset.id, a => { a.monthlyMaintenance = Number(e.target.value) || 0; })}
                className="h-8 text-right" />
            </div>
            {asset.propertyMode === 'rental' && (
              <div>
                <Label className="block mb-1">月租金 ¥</Label>
                <Input type="number" step={500} min={0} value={asset.monthlyRent ?? 0}
                  onChange={e => updateAsset(asset.id, a => { a.monthlyRent = Number(e.target.value) || 0; })}
                  className="h-8 text-right" />
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Input type="number" step={1000} min={0} value={asset.amountCny ?? 0}
                onChange={e => updateAsset(asset.id, a => { a.amountCny = Number(e.target.value) || 0; })}
                className="h-8 text-right" />
            </div>
            <div>
              <Label className="block mb-1">月定投 ¥</Label>
              <Input type="number" step={100} min={0} value={asset.dcaAmount ?? 0}
                onChange={e => updateAsset(asset.id, a => { a.dcaAmount = Number(e.target.value) || 0; })}
                className="h-8 text-right" />
            </div>
          </div>
        )}
        ...
        <div className="flex items-end gap-3 pt-3 mt-3 border-t border-border">
          <div className="flex-1">
            ...
            <Input type="number" step={0.1} min={-50} max={100}
              value={asset.expectedReturn != null ? (asset.expectedReturn * 100).toFixed(1) : ((def?.ret ?? 0.07) * 100).toFixed(1)}
              onChange={e => updateAsset(asset.id, a => { a.expectedReturn = (Number(e.target.value) || 0) / 100; })}
              className="h-8 text-right" />
          </div>
          <div className="flex-1">
            <div className="flex justify-between mb-1">
            ...
            <Input type="number" step={0.5} min={0} max={200}
              value={asset.volatility != null ? (asset.volatility * 100).toFixed(1) : ((def?.vol ?? 0.15) * 100).toFixed(1)}
              onChange={e => updateAsset(asset.id, a => { a.volatility = (Number(e.target.value) || 0) / 100; })}
              className="h-8 text-right" />
          </div>
        </div>
      </CardContent>
  ...
  return (
    <div className="px-6 py-5 overflow-y-auto h-full max-w-5xl">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-xl font-semibold text-text-1 mb-1 tracking-tight">资产</h2>
