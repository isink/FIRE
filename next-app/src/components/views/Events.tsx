...
export function Events() {
  ...
  return (
    <div className="px-6 py-5 overflow-y-auto h-full max-w-5xl">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-text-1 mb-1 tracking-tight">时间线事件</h2>
        ...
        <Card>
          {events.map((ev: any, i: number) => {
            ...
            return (
              ...
              >
                <div className="text-md mono font-semibold text-text-1 w-14">{ev.year}</div>
                <div className="flex-1 flex items-center gap-2">
                  <span className="text-xs px-1.5 py-0.5 rounded-sm bg-surface-sunken text-text-3 font-medium ring-1 ring-border/60">{typeTag}</span>
                  <span className="font-medium text-text-1">{ev.name}</span>
                </div>
                {/* 红=正/收入, 绿=负/支出 per 中国惯例 */}
                <div className={cn('mono text-base font-semibold', isPositive ? 'text-gain' : 'text-loss')}>{impactText}</div>
