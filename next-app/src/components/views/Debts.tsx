  ...
  return (
    <div className="px-6 py-5 overflow-y-auto h-full max-w-5xl">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-xl font-semibold text-text-1 mb-1 tracking-tight">债务</h2>
              ...
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                  <div>
                    ...
                    <Input type="number" step={10000} min={0} value={d.principal ?? 0}
                      onChange={e => updateLiability(d.id, x => { x.principal = Number(e.target.value) || 0; })}
                      className="h-8 text-right" />
                  </div>
                  <div>
                    <Label className="block mb-1">年利率 %</Label>
                    <Input type="number" step={0.1} min={0} max={20}
                      value={((d.rate || 0) * 100).toFixed(2)}
                      onChange={e => updateLiability(d.id, x => { x.rate = (Number(e.target.value) || 0) / 100; })}
                      className="h-8 text-right" />
                  </div>
                  <div>
                    <Label className="block mb-1">年限</Label>
                    <Input type="number" min={1} max={50} value={d.years ?? 25}
                      onChange={e => updateLiability(d.id, x => { x.years = Number(e.target.value) || 0; })}
                      className="h-8 text-right" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input type="number" min={2000} max={2100} value={d.startYear ?? 2024}
                      onChange={e => updateLiability(d.id, x => { x.startYear = Number(e.target.value) || 0; })}
                      className="h-8 text-right" />
                  </div>
                </div>
              </CardContent>
