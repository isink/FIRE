  ...
  return (
    <div className="px-6 py-5 overflow-y-auto h-full max-w-5xl">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-xl font-semibold text-text-1 mb-1 tracking-tight">支出分类</h2>
          ...
          {cats.map((c: any, i: number) => (
            ...
            >
              ...
              <Input
                ...
                value={c.monthly || 0}
                onChange={e => updateCat(c.id, x => { x.monthly = Number(e.target.value) || 0; })}
                className="h-8 text-right"
              />
              <Input
                type="number" step={0.1} min={-5} max={20}
                value={((c.inflationRate ?? 0.025) * 100).toFixed(1)}
                onChange={e => updateCat(c.id, x => { x.inflationRate = (Number(e.target.value) || 0) / 100; })}
                className="h-8 text-right"
              />
