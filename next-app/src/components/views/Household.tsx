    ...
    <div className="px-6 py-5 overflow-y-auto h-full max-w-5xl">
      <div className="mb-6">
        <div className="flex items-center gap-2.5 mb-1">
          <h2 className="text-xl font-semibold text-text-1 tracking-tight">家庭成员</h2>
              ...
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <Input
                      type="number" min={1940} max={2025} step={1}
                      value={person.birthYear || _thisYear - 30}
                      onChange={e => updatePerson(person.id, p => { p.birthYear = Number(e.target.value) || 0; })}
                    />
                  </div>
                  <div>
                    <Label className="block mb-1.5">计划退休年</Label>
                    <Input
                      type="number" min={_thisYear} max={2100} step={1}
                      value={person.retireYear || _thisYear + 25}
                      onChange={e => updatePerson(person.id, p => { p.retireYear = Number(e.target.value) || 0; })}
                    />
