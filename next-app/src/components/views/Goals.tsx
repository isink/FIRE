...
function GoalCard({ goal, baselineSim }: { goal: any; baselineSim: any }) {
 ...
 <span className="font-medium text-text-1">{goal.name || '目标'}</span>
 ...
 <div>
 <div className="text-xs text-text-3">目标金额</div>
 <div className="text-lg font-medium mono text-text-1 mt-1">¥{fmtCompact(goal.amount || 0)}</div>
 </div>
 <div>
 <div className="text-xs text-text-3">目标年份</div>
 <div className="text-lg font-medium mono text-text-1 mt-1">{goal.year}</div>
 <div className="text-xs text-text-3">{yearsAway > 0 ? yearsAway + ' 年后' : yearsAway === 0 ? '今年' : '已过'}</div>
 </div>
 <div>
 <div className="text-xs text-text-3">届时 P50</div>
 <div className="text-lg font-medium mono text-text-1 mt-1">{p50 != null ? '¥' + fmtCompact(p50) : '—'}</div>
 <div className="text-xs text-text-3">无目标基线</div>
 </div>
 <div>
 <div className="text-xs text-text-3">承担能力</div>
 <div className={cn('inline-block mt-1 text-sm font-medium px-2 py-1 rounded-sm', fundedCls)}>{fundedStatus}</div>
