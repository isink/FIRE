...
export function Sankey() {
 ...
 const amt = (Number(s.monthlyAmount) || 0) / (s.freq === 'year' ? 12 : 1);
 if (s.type === 'gross') {
 ...
 return (
 <div className={PANEL}>
 <div className="text-md font-medium text-text-1 mb-2 tracking-tight">月度现金流桑基</div>
 ...
 <div className="text-md font-medium text-text-1 tracking-tight">月度现金流桑基</div>
 <div className="text-sm text-text-3">
 税前 <strong className="mono text-text-1 font-medium">¥{fmt(data.gross)}</strong>
 <span className="mx-1.5 text-border-strong">·</span>
 净储蓄 <strong className="mono text-gain font-medium">¥{fmt(data.savings)}</strong>
 <span className="mx-1.5 text-border-strong">·</span>
