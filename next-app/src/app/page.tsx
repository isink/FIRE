    ...
    <div className="min-h-screen bg-surface text-text-1">
      ...
      <header className="sticky top-0 z-40 bg-surface/85 backdrop-blur-md border-b border-border">
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-7 h-7 bg-primary text-primary-foreground font-bold rounded-md grid place-items-center text-sm">F</div>
            <span className="font-semibold tracking-tight">FIRE Planner</span>
          </Link>
      ...
      <section className="relative">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
          <div className="mx-auto max-w-3xl text-center">
            <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium text-primary ring-1 ring-primary/20 bg-primary/[0.06] mb-7">
              <span className="w-1.5 h-1.5 rounded-full bg-primary"></span>
              14 个分析维度全部用中国规则建模
            </div>
            <h1 className="text-hero sm:text-[3.25rem] font-bold tracking-tight leading-[1.1] text-text-1">
              算清楚你<span className="text-primary">几岁</span>能 FIRE
            </h1>
            <p className="mt-6 text-lg leading-relaxed text-text-2 max-w-2xl mx-auto">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  {[
                    ...
                    <div key={i} className={'rounded-md p-3 ring-1 ' + (m.accent ? 'bg-primary/[0.04] ring-primary/25' : 'bg-surface-sunken ring-border/60')}>
                      <div className={'text-xs uppercase tracking-wider ' + (m.accent ? 'text-primary' : 'text-text-3')}>{m.k}</div>
                      <div className={'text-2xl font-bold mono mt-1 ' + (m.accent ? 'text-primary' : 'text-text-1')}>{m.v}</div>
      ...
      <section id="features" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div className="max-w-3xl mx-auto text-center mb-14">
          <h2 className="text-sm font-semibold text-primary tracking-wider uppercase">能力</h2>
          <p className="mt-3 text-2xl font-bold tracking-tight text-text-1">这是规划器，不是计算器</p>
          ...
          {FEATURES.map((f, i) => {
            ...
            return (
              <div key={i} className="rounded-lg bg-surface ring-1 ring-border p-5 hover:ring-border-strong hover:shadow-e1 transition-all duration-fast">
                ...
                <h3 className="text-md font-semibold mb-1.5 text-text-1">{f.title}</h3>
      ...
      <section id="pricing" className="bg-canvas border-y border-border">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="max-w-3xl mx-auto text-center mb-14">
            <h2 className="text-sm font-semibold text-primary tracking-wider uppercase">定价</h2>
            <p className="mt-3 text-2xl font-bold tracking-tight text-text-1">早期用户全部免费</p>
            ...
            {PRICING.map((p, i) => (
              <div key={i} className={'relative rounded-xl bg-surface p-7 flex flex-col ' + (p.featured ? 'ring-2 ring-primary shadow-e2' : 'ring-1 ring-border')}>
                ...
                <p className="mt-4 flex items-baseline">
                  <span className="text-2xl font-bold tracking-tight mono text-text-1">{p.price}</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            ...
            <div className="flex gap-12 text-base text-text-3">
              <div className="space-y-2">
                <div className="font-semibold text-text-1">产品</div>
              ...
              <div className="space-y-2">
                <div className="font-semibold text-text-1">法律</div>
